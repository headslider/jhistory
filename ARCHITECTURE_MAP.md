...
- スマホ用の子カテゴリー圧縮は `.event-subcategories .subcategory-head p:not(.eyebrow)`、`.event-subcategory-card .subcategory-card-body`、`.subcategory-image`、`.subcategory-description`、`.tag-row` で制御する。閉じた状態では本文プレビューを出さず、画像・時代を除いたタイトル情報を優先する。タップで `.is-description-expanded` になった時だけ、本文をカード下の全幅領域に表示し、画像もカード横幅いっぱいへ拡大する。閉じた状態のサムネイルは一覧性を優先して `object-position: center` に固定し、展開時だけ `.subcategory-image-up` / `.subcategory-image-down` の上下補正を再有効化する。 時代カード内の人物一覧 `details.era-people` は `mobile era people list repair` がスマホ最終上書きで、summaryを「タイトル・人数・シェブロン」の3列、`.mini-people` を2列グリッド、380px以下を1列にして、人物名・ルビ・アイコンが横幅不足で崩れないよう制御する。
- 2026-07-21時点で、スマホ下部固定ナビは不要導線として `index.html` / `script.js` / `styles.css` から削除済み。スマホの主要操作は左上メニューと右上オプションメニューで行う。
- 検証は `scripts/verify-static.ps1`、同梱Nodeの `--check`、Chrome実表示の430px幅CDP計測で行う。左側の年代札は疑似要素の表示X座標が画面内になるよう補正する。2026-07-28の世界史同期後、スマホの基本上書きは `styles.css` 末尾の `mobile timeline/card layout consolidation` で管理し、`--mobile-line-x: 36px`、`--mobile-card-left: 58px` を基準にする。さらに `mobile timeline fine tune` が最終上書きとして、大カテゴリーの年代札 `.era-group::after` を少し上へ置く `top`、大カテゴリータイトル・サブタイトルの幅いっぱいの折り返し、時代カード見出し `.era-head h2` のスマホ用縮小、時代カード左アイコン `.era::before` の縦ライン中央合わせを制御する。大カテゴリーの年代札 `.era-group::after` はカード横ではなく、大カテゴリー同士の上下余白に入るようスマホだけ `margin-top` と `top` を調整する。各時代カードの年代札 `.era::after` もスマホではカード横ではなく時代カード上の余白へ配置し、`.era` の `margin-top` と `.era::after top` でカード本体と重ならないよう制御する。スクリーンショットは `reports/visual-checks/mobile-timeline-20260719-era-card-year-top.png`。
...
## 2026-07-30 系譜本文リンク監査と参照子カテゴリー調整
- オプションの各系譜本文を既存人物カード・アクションカード・子カテゴリーに照合し、本文の流れを崩さない範囲で参照子カテゴリーを補正した。`戦争の系譜` には鎖国、`武士の系譜` には廃藩置県、`栄枯盛衰の系譜` には大化の改新・関ヶ原の戦い・開国を追加し、本文で示す転換点とカード列の対応を強めた。
- 本文中の `北条氏` は、鎌倉幕府の執権北条氏を指す文脈で `鎌倉北条氏` アクションカードへ接続するため、`script.js` の `inlineLinkAliases` に限定的な別名リンクを追加した。漢字隣接許可は付けず、`北条氏康` など別人物名への誤リンクを避ける。
- 監査結果は `reports/lineage-link-audit.json` に保存し、同じ監査を繰り返す場合はこの結果と `scripts/verify-static.ps1` の検証を優先する。
